...
**safeT란? 교통 안전을 의미하는 'safety'와 전동 킥보드 손잡이를 표현한 'T'가 만나 만들어진, 전동 킥보드 안전 시스템입니다!** </br>
 ...
 더불어 안전모 착용 여부와 2인 이상 탑승 여부, 도로 구별 감지 시스템 등을 통해 안전성을 대폭 강화한 특징을 지니고 있습니다.</br><br><br>
...
> **[ACK 2024 학술발표대회 논문집 (31권 2호) : AI 기술을 이용한 스마트 전동 킥보드 안전 시스템](https://doi.org/10.3745/PKIPS.y2024m10a.1043)**
...
킥보드 반납 시 점자 블록, 횡단보도 감지를 통해 주변 상황을 검출하여 주차 공간을 자동 판별하기 위해 YOLOv8 모델을 활용하여 학습을 진행하였다.<br>
학습 데이터셋은 roboflow 제공 데이터셋과 추가로 직접 라벨링한 데이터셋을 활용하였다.<br>
block, crosswalk 클래스로 나누어 학습하여 점자블록, 횡단보도 감지 시 반납 불가 처리 및 경고 메시지 출력으로 올바른 주차를 유도하도록 하였다.<br><br>
